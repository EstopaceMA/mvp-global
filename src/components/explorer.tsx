...
export function Explorer() {
  ...
  const result = useDirectoryResult(filters);
  const countsPending = !result && hasFilters(filters);
  const counts = result?.counts ?? (countsPending ? EMPTY_COUNTS : manifest.counts);
    ...
    <Sheet open={showResults} onOpenChange={onOpenChange} modal={mobile}>
      <SheetContent side={mobile ? "bottom" : "right"} className="country-sheet" showCloseButton={false}
        ...
        onCloseAutoFocus={event => {
          ...
          // A visitor may start another interaction during the exit animation.
          // Return focus only if it has not already moved outside the closing sheet.
          const active = document.activeElement;
          if (!active || active === document.body || active.closest(".country-sheet")) resultsButton.current?.focus();
        }}>
        ...
        <div className="sheet-scroll" key={`${filters.country}-${filters.page}`}><Results result={result} compact onInteract={() => { setMobileEditing(false); setOpened(true); }}/></div>
      </SheetContent>
    </Sheet>
  </main>;
}
